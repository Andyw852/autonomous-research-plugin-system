...
```bash
# From the RDKit SKILL directory
bash scripts/setup_env.sh
```

The private `.venv` is synchronized from `requirements.lock`. The PyPI package is
now `rdkit`; `rdkit-pypi` is the legacy name for older releases.
