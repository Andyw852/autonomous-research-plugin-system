...
This reference targets **PyTDC 1.1.14**, verified 2026-07-23. Oracle names and
...
The skill ships a pre-seeded local cache at `.pytdc-oracles/oracle/`:

```text
.pytdc-oracles/oracle/drd2_current.pkl
.pytdc-oracles/oracle/fpscores.pkl
.pytdc-oracles/oracle/gsk3b_current.pkl
.pytdc-oracles/oracle/jnk3_current.pkl
```

Keep the exact filenames. PyTDC loads these hard-coded paths relative to the
working directory, and the `_current` variants are the ones selected by
PyTDC 1.1.14 with scikit-learn >= 0.24. When the expected file exists, oracle
construction reports `Found local copy...` and performs no network request.
Origin and checksums are recorded in `.pytdc-oracles/README.md`.
...
```bash
.venv/bin/python scripts/molecular_generation.py score \
  --oracle QED \
  --smiles "CCO"
```
...
```bash
.venv/bin/python scripts/discover_metadata.py --kind datasets --task MolGen
```

Plan a random split:

```bash
.venv/bin/python scripts/molecular_generation.py dataset \
  --dataset MOSES \
  --seed 42 \
