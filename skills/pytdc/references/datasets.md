...
This reference targets the **PyTDC 1.1.14** source distribution, verified
...
```bash
.venv/bin/python scripts/discover_metadata.py --kind tasks --limit 100

.venv/bin/python scripts/discover_metadata.py \
  --kind datasets --task DTI --limit 100
```
...
```bash
.venv/bin/python scripts/cache_audit.py --cache-dir .pytdc-data --largest 20
```
