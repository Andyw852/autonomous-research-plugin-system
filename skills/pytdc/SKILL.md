...
metadata:
  ...
  compatibility: Requires uv, CPython 3.11, PyTDC 1.1.14, and setuptools 80.9.0 for its legacy pkg_resources runtime import. PyTDC 1.1.14 pins scikit-learn==1.2.2, which is required to load the old-format GSK3B/JNK3 oracle checkpoints. Dataset, benchmark, checkpoint, and remote-oracle operations require network/storage review and explicit user approval.
---
...
boundary of what PyTDC supports.

- **Can construct**: task/dataset registries and split structures from approved
  datasets; benchmark groups; evaluation sets for given model predictions. PyTDC
  is a data and evaluation platform, not a molecular generator.
- **Can compute**: registered evaluator metrics on submitted predictions; bounded
  model-backed oracle predictions (e.g. docking or ML oracles) after approval;
  dataset, split, and benchmark metadata.
- **Can perturb**: task/dataset selection, split method and parameters, oracle
  selection, metric selection, and prediction inputs, within the documented
  PyTDC APIs.
- **Cannot do**: ab initio or force-field simulation, generation of novel
  molecules, experimental validation, or any computation not exposed by the
  installed PyTDC registry.
- **Protocol identity**: record PyTDC version, environment pins, dataset/task
  name, split method and seed, oracle checkpoint identifiers, metric names, and
  cache locations for every result.
- **Randomness**: dataset splits and stochastic oracles use explicit seeds;
  fixed-seed reruns are reproducibility checks, not independent physical
  replicates.
- **Cost ladder**: metadata discovery is low cost; dataset download and parsing
  are medium cost; remote or model-backed oracles are high cost and require
...
- Environment pins: **PyTDC 1.1.14** (its scikit-learn==1.2.2 requirement loads the GSK3B/JNK3 checkpoints)
...
From this SKILL directory, create and synchronize its private CPython 3.11
...
with `uv pip sync`, then validates the installed environment. Do not install PyTDC
into the project interpreter or another SKILL's environment. Before
synchronization, inspect the plan when cost or storage is material:

```bash
uv pip sync --dry-run --python .venv/bin/python requirements.lock
```

The resolved environment includes large scientific/ML dependencies and may occupy
multiple gigabytes before any dataset or oracle checkpoint is downloaded. Review
available disk and network policy first. Run bundled commands with this exact
interpreter; do not replace it with an ephemeral `uv run --with` environment:

```bash
.venv/bin/python scripts/discover_metadata.py --kind tasks
```

PyTDC 1.1.15 unconditionally declares the Python 3.6 backport `dataclasses<1.0`.
Its locked `dataclasses==0.8` metadata therefore fails a generic compatibility
check on CPython 3.11, while the runtime correctly imports the CPython standard
library module. `setup_env.sh` permits only this one verified metadata defect,
checks that `dataclasses` resolves outside `site-packages`, and fails on any other
...
- Pre-seeded local checkpoints live at `.pytdc-oracles/oracle/` with the exact
  upstream filenames (`drd2_current.pkl`, `fpscores.pkl`, `gsk3b_current.pkl`,
  `jnk3_current.pkl`). When the expected file exists, PyTDC reports
  `Found local copy...` and does not download. Origin and checksums are recorded
  in `.pytdc-oracles/README.md`.
...
```bash
.venv/bin/python scripts/discover_metadata.py \
  --kind datasets --task ADME --limit 50

.venv/bin/python scripts/discover_metadata.py --kind benchmarks --limit 50

.venv/bin/python scripts/discover_metadata.py --kind evaluators --limit 100
```
...
```bash
.venv/bin/python scripts/load_and_split_data.py \
...
```bash
.venv/bin/python scripts/discover_metadata.py --kind oracles --limit 100
```

Plan bounded local QED scoring:

```bash
.venv/bin/python scripts/molecular_generation.py \
  score --oracle QED --smiles CCO
```
...
- `scripts/setup_env.sh` — idempotent private `.venv` creation and locked sync
